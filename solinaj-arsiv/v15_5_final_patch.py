...
def apply_update(source_text):
    source_text = re.sub(r'APP_NAME\s*=\s*"Solinaj Arşivleyici V[^"]+"', 'APP_NAME = "Solinaj Arşivleyici V15.5"', source_text, count=1)
    source_text = re.sub(r'APP_VERSION\s*=\s*"[^"]+"', 'APP_VERSION = "15.5"', source_text, count=1)
...
def stop_scan(self):
    if not self.running:
        ...
        return
    self.stop_event.set()
    self.status.set("Durdurma istendi...")
    try:
        self.stop_btn.configure(state="disabled")
    except Exception:
        pass
    self.add_log_ui("DURDUR komutu alındı. Aktif kopyalama güvenli şekilde kesiliyor...")
"""
    ...
    try:
        with src.open("rb") as fi, dst.open("wb") as fo:
            while True:
                ...
                self.after(0,lambda p=pct:self.progress.configure(mode="determinate",maximum=100,value=p))
        try:
...
def _scan_worker(self):
    try:
        ...
        files=[]
        self.after(0,lambda:self.progress.configure(mode="indeterminate",maximum=100,value=0))
...
"""
    ...
    compile(source_text, "<solinaj_arsivleyici_v15_5>", "exec")
